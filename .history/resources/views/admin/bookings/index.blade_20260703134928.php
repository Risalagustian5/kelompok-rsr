        ...
        <div class="table-wrapper">
          <table>
            ...
            <tbody>
              ...
              <tr>
                {{-- Menggunakan akses array dengan fallback jika penamaan join tabel di Supabase berbentuk plural/singular --}}
                <td>{{ $b['user']['name'] ?? ($b['users']['name'] ?? 'Unknown User') }}</td>
                <td>{{ $b['user']['email'] ?? ($b['users']['email'] ?? '-') }}</td>
                <td>{{ $b['villa']['nama_villa'] ?? ($b['villas']['nama_villa'] ?? '-') }}</td>
                <td>{{ $b['check_in'] ?? '-' }}</td>
                <td>{{ $b['check_out'] ?? '-' }}</td>
              ...
              @empty
              <tr>
                <td colspan="7" style="text-align:center; padding:20px;">Belum ada pesanan.</td>
              </tr>
              @endforelse
            </tbody>
          </table>
