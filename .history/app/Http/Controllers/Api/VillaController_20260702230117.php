...
class VillaController extends Controller
{
    ...
    public function historyBooking() {
        $response = $this->client()->get(
            $this->supabaseUrl . '/rest/v1/bookings?select=*,villa:villas(id,nama_villa,lokasi,harga,foto,deskripsi),user:users(id,name,email)&user_id=eq.' . auth()->id()
        );
        ...
        $bookings = array_map(function($b) {
            $b = (object) $b;
            if (isset($b->villa)) $b->villa = (object) $b->villa;
            if (isset($b->user))  $b->user  = (object) $b->user;
            return $b;
        }, $data);
    ...
    public function adminBookings() {
        $response = $this->client()->get(
            $this->supabaseUrl . '/rest/v1/bookings?select=*,villa:villas(id,nama_villa,lokasi,harga,foto,deskripsi),user:users(id,name,email)'
        );
    ...
    // Method baru: menghapus permanen data booking dari Supabase.
    // Ini yang tadinya hilang dan menyebabkan error "Call to undefined method destroyBooking()".
    public function destroyBooking($id) {
        $response = $this->client()->delete($this->supabaseUrl . '/rest/v1/bookings?id=eq.' . $id);

        if ($response->successful()) {
            return redirect()->route('admin.bookings.index')->with('success', 'Booking berhasil dihapus');
        }

        return back()->withErrors(['msg' => 'Gagal menghapus booking.']);
    }
}