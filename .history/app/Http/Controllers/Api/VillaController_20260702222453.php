...
class VillaController extends Controller
{
    ...
    public function historyBooking() {
        // Ambil booking milik user + join ke tabel villas agar detail villa ikut terbawa
        $response = $this->client()->get(
            $this->supabaseUrl . '/rest/v1/bookings?select=*,villa:villas(id,nama_villa,lokasi,harga,foto)&user_id=eq.' . auth()->id()
        );

        $data     = $response->json();
        $bookings = array_map(fn($b) => (object) $b, $data);
    ...
    public function adminBookings() {
        $response = $this->client()->get($this->supabaseUrl . '/rest/v1/bookings?select=*,villa:villas(id,nama_villa,lokasi,harga,foto)');
